...
--  © 2022 Treebonker, All rights reserved.
--
...
function module.OnUpdateTimer()
	...
	elseif (currentCharges ~= lastCharges) then
		...
		if (currentCharges < maxCharges) then
			...
			Timer = C_Timer.NewTicker(math.abs(cooldownDuration - GetTime() + cooldownStart), module.OnUpdateTimer, 1);
		end
...
local function CreateBrez()
	...
	icon.Count:SetPoint("BOTTOMRIGHT", icon, 0, 0);
	...
	icon:SetScript("OnMouseDown", OmniCD_AnchorOnMouseDown);
	icon:SetScript("OnMouseUp", OmniCD_AnchorOnMouseUp);
end
