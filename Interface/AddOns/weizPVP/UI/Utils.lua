...
function NS.SaveCoreFramePosition()
  NS.Options.Frames.X, NS.Options.Frames.Y, NS.Options.Frames.Width, _, NS.Options.Frames.Scale, NS.Options.Frames.Point =
    GetFramePosition(weizPVP_CoreFrame)
...
function NS.FormatPlayerNameAndRealm(playerName, GUID)
  ...
  if NS.PlayerDB[playerName] then
    charNameAndRealm =
      WrapTextInColorCode(gsub(playerName, "-(.*)", ""), select(4, GetClassColor(NS.PlayerDB[playerName].C)))
    charNameAndRealm =
      charNameAndRealm .. " |cffbbbbbb-|r " .. NS.ColorsLUT["realm"]:WrapTextInColorCode(gsub(playerName, "^(.*-)", ""))
  else
    charNameAndRealm =
      WrapTextInColorCode(
      gsub(playerName, "-(.*)", ""),
